...
class ddpg:
    def __init__(self, env_fn, hid=256, layers=2, gamma=0.99, seed=0, buffer_size=int(1e6), polyak=0.995,
                 ...
                 update_after=1000, noise_scale=0.1, start_steps=10000):
        ...
        # create actor-critic
        self.ac = ac_core.MLPActorCritic(self.obs_dim[0], self.act_dim[0], [hid] * layers, activation, self.act_limit)
        ...
        # Count variables
        var_counts = tuple(ac_core.count_vars(module) for module in [self.ac.pi, self.ac.q])
    ...
    def update(self):
        ...
        for i in range(self.update_every):
            ...
            with torch.no_grad():
                q_target = self.ac_target.q(o2, self.ac_target.pi(o2))
            ...
            # freeze params in q-network to speed up
            for params in self.ac.q.parameters():
                params.requires_grad = False

            # Freeze Q-network so you don't waste computational effort
            # computing gradients for it during the policy learning step.
            self.pi_optimizer.zero_grad()
            loss_pi = -self.ac.q(o, self.ac.pi(o)).mean()
            loss_pi.backward()
        ...
        self.logger.store(LossPi=loss_pi.item(), LossQ=loss_q.item(), QVals=q.detach().numpy())
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_name', type=str, default='ddpg_Walker2d-v3')
    ...
    agent = ddpg(lambda: gym.make(args.env), hid=args.hid, layers=args.layers, seed=args.seed,

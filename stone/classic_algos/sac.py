...
class sac:
    def __init__(self, env_fn, hid=256, layers=2, gamma=0.99, seed=0, buffer_size=int(1e6), polyak=0.995,
                 ...
                 update_after=1000, alpha=0.2, start_steps=10000, entropy_adjust=True):
        ...
        # create actor-critic
        self.ac = ac_core.MLPSacActorCritic(self.obs_dim[0], self.act_dim[0], [hid] * layers, activation,
                                            self.act_limit)
        ...
        self.alpha, self.entropy_adjust, self.dual_optimizer = alpha, entropy_adjust, None
        self.target_ent = -np.prod(self.act_dim)
        if self.entropy_adjust:
            self.alpha = torch.nn.Parameter(torch.as_tensor(alpha, dtype=torch.float32), requires_grad=True)
            self.dual_optimizer = Adam([self.alpha], lr=pi_lr)
    ...
    def test_policy(self, max_ep_len, num_test_episodes):
        for e in range(num_test_episodes):
            ...
            while not (d or (ep_len == max_ep_len)):
                a = self.make_action(torch.as_tensor(o, dtype=torch.float32), True)
        ...
        for t in range(epochs * steps_per_epoch):
            ...
            if (t + 1) % steps_per_epoch == 0:
                ...
                if self.entropy_adjust:
                    ...
                    logger.log_tabular('lossDual', average_only=True)
    ...
    def compute_loss_q(self, data):
        ...
        with torch.no_grad():
            ...
            backup = r + self.gamma * (1 - d) * (q_target - self.alpha * logp_a2)
    ...
    def compute_loss_dual(self, logp):
        ...
        loss_dual = -self.alpha * (logp + self.target_ent).mean()
        return loss_dual
    ...
    def update(self):
        ...
        for i in range(self.update_every):
            ...
            # Optionally update dual variable
            if self.entropy_adjust:
                self.dual_optimizer.zero_grad()
                loss_dual = self.compute_loss_dual(pi_info['LogPi'])
                loss_dual.backward()
                self.dual_optimizer.step()

                # Record things
                self.logger.store(lossDual=loss_dual.item(), alpha=self.alpha)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ent_adjust', type=bool, default=True)
    # parser.add_argument('--exp_name', type=str, default='sac_HalfCheetah_auto_ent_adjust')
    parser.add_argument('--exp_name', type=str, default='sac_Humanoid_autoEnt')
    ...
    agent = sac(lambda: gym.make(args.env), hid=args.hid, layers=args.layers, seed=args.seed,
                pi_lr=args.pi_lr, q_lr=args.q_lr, logger_kwargs=logger_kwargs, entropy_adjust=args.ent_adjust)
